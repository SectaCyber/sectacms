...
Overview
---------
Wagtail Secta is an extension of Wagtail. You can further customize your site by overriding the
...
in here will override the default sectacms templates if they follow the same name and directory
structure. This uses the standard Django template rendering engine. For example, to change the
formatting of the article page, copy ``sectacms/templates/sectacms/pages/article_page.html``
to ``website/templates/sectacms/pages/article_page.html`` and modify it.
The `source code for built-in templates can be found on GitHub
<https://github.com/SectaCyber/sectacms/blob/main/sectacms/templates/sectacms/>`_.


Example 1: Navbar Customization
-------------------------------

The built-in template for the navbar can be found in ``templates/sectacms/snippets/navbar.html``. This
file may not actually be in your installation folders for your site; however, you can see its contents
by visiting the Wagtail Secta source code here: `navbar.html <https://github.com/SectaCyber/sectacms/blob/main/sectacms/templates/sectacms/snippets/navbar.html>`_.
...
called ``templates``. In there are two folders as well: ``website`` and ``sectacms``. The ``sectacms`` template
folder is likely empty at this point because the CMS is pulling in the default templates from source, but you can
add templates to the ``sectacms`` folder **if you are overriding the default templates**.
...
.. note::
    Adding templates to the ``sectacms`` templates folder does not change the default templates
    throughout all of Wagtail Secta but does override those specific templates for your website app.

Your ``website`` folder currently only has a folder for ``sectacms`` in the ``templates`` folder.
You can add a new ``website`` folder in ``templates`` (because we will use it in another tutorial),
but for now, you will want to add a ``snippets`` folder inside the ``templates\sectacms`` folder
...
override the navbar template. Now you should have ``templates\sectacms\snippets``. Navigate to
...
5. The 2-tiered navbar will have two navbars on top of each other but one will only have the
   ...
   However, we did keep the ``{% if settings.sectacms.LayoutSettings.logo %} {% endif %}`` block because we want
...
6. Now we can include the code block for the normal navbar beneath it. Place this code below the ``</nav>`` in
   ...
   .. code-block:: Django
      ...
      {% endif %}
         ...
         <form class="ml-auto form-inline" action="{% url 'Secta_search' %}" method="GET">
...
For our custom navbar, we needed to un-check the "fixed navbar" option in the CMS via **Settings > Secta Settings** in
