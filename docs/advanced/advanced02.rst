...
You should already see a few page models in there from Wagtail Secta, as well as imports at the top from the
...
* We our extending the ``sectaWebPage`` model which is why it is wrapped in parentheses after we name our page model.
...
.. code:: Django
    ...
    {% extends "sectacms/pages/web_page.html" %}
    {% load wagtailcore_tags wagtailimages_tags sectacms_tags %}
...
publish it. The page does not have a template made; however, it uses the basic secta Web Page so it will display
something.
...
The standard secta Web Page template has an ``{% if %} {% else %}`` statement regarding cover images that says to show the page title when a cover image
