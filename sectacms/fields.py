...
from sectacms.widgets import ColorPickerWidget


class sectaStreamField(StreamField):
    """
    ...
    migration problems (most commonly: a client overrides Secta_FRONTEND_*,
    which changes a string used in a concrete model, which triggers a migration
    back in sectacms). Eliminiating the blocks from the deconstructed
    ...
    Internally, we should ALWAYS use sectaStreamField on CONCRETE models,
    ...
    default. The client may then decide to use sectaStreamField if they are
    annoyed by the big migrations.

    CAVEAT EMPTOR:

    Client sites built with Secta may use this in place of the Wagtail
    StreamField, in order to avoid huge migration files. However, note that it
    will not be possible to mine data out of a sectaStreamField during a
    ...
    def deconstruct(self):
        ...
        blocks are nested within the StreamField::
            ...
            ("body", sectacms.fields.sectaStreamField([]))
