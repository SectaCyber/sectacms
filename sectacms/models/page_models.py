"""
Base and abstract pages used in Wagtail Secta.
"""
...
from sectacms import utils
from sectacms.blocks import (
    CONTENT_STREAMBLOCKS,
    LAYOUT_STREAMBLOCKS,
    STREAMFORM_BLOCKS,
    ContentWallBlock,
)
from sectacms.fields import sectaStreamField, ColorField
from sectacms.forms import sectaFormBuilder, sectaSubmissionsListView
from sectacms.models.snippet_models import ClassifierTerm
from sectacms.models.wagtailsettings_models import LayoutSettings
from sectacms.wagtail_flexible_forms.blocks import (
    FormFieldBlock,
    FormStepBlock,
)
from sectacms.wagtail_flexible_forms.models import (
    Step,
    Steps,
    StreamFormMixin,
...
from sectacms.widgets import ClassifierSelectWidget
...
logger = logging.getLogger("sectacms")
...
def get_page_models():
    return Secta_PAGE_MODELS


class sectaPageMeta(PageBase):
    def __init__(cls, name, bases, dct):
        super().__init__(name, bases, dct)
        if not cls._meta.abstract:
            Secta_PAGE_MODELS.append(cls)


class sectaTag(TaggedItemBase):
    class Meta:
        verbose_name = _("secta Tag")

    content_object = ParentalKey(
        "sectacms.sectaPage", related_name="tagged_items"
    )


class sectaPage(WagtailCacheMixin, SeoMixin, Page, metaclass=sectaPageMeta):
    """
    ...
    search_template = "sectacms/pages/search_result.html"

    # Template used for related pages, Latest Pages block, and Page Preview block.
    miniview_template = "sectacms/pages/page.mini.html"
    ...
    index_query_pagemodel = "sectacms.sectaPage"
    ...
    content_walls = sectaStreamField(
    ...
    def __init__(self, *args, **kwargs):
        ...
        template_choices = Secta_settings.Secta_FRONTEND_TEMPLATES_PAGES.get(
            "*", []
        ) + Secta_settings.Secta_FRONTEND_TEMPLATES_PAGES.get(klassname, [])
    ...
    def get_index_children(self):
        ...
        if self.index_order_by_classifier:
            terms = ClassifierTerm.objects.filter(
                ...
                # Reverse ManyToMany of `sectapage.classifier_terms`.
                sectapage=models.OuterRef("pk"),
            )[:1]
...
class sectaWebPage(sectaPage):
    ...
    # Search fields
    search_fields = sectaPage.search_fields + [index.SearchField("body")]
...
class sectaArticlePage(sectaWebPage):
    """
    Article, suitable for news or blog content.
    """

    class Meta:
        verbose_name = _("secta Article")
        abstract = True
    ...
    def get_author_name(self):
        warnings.warn(
            (
                "sectaArticlePage.get_author_name has been replaced with "
                "sectaArticlePage.seo_author"
            ),
            DeprecationWarning,
        )
    ...
    def get_pub_date(self):
        warnings.warn(
            (
                "sectaArticlePage.get_pub_date has been replaced with "
                "sectaArticlePage.seo_published_at"
            ),
            DeprecationWarning,
        )
    ...
    def get_description(self):
        warnings.warn(
            (
                "sectaArticlePage.get_description has been replaced with "
                "sectaArticlePage.seo_description"
            ),
            DeprecationWarning,
        )
    ...
    search_fields = sectaWebPage.search_fields + [
    ...
    content_panels = sectaWebPage.content_panels + [
        FieldPanel("caption"),
        MultiFieldPanel(
            [
                FieldPanel("author"),
...
class sectaArticleIndexPage(sectaWebPage):
    """
    Shows a list of article sub-pages.
    """

    class Meta:
        verbose_name = _("secta Article Index Page")
        abstract = True
    ...
    ) + sectaWebPage.index_order_by_choices
    ...
    layout_panels = sectaWebPage.layout_panels + [
        MultiFieldPanel(
            [
...
class sectaEventPage(sectaWebPage, BaseEvent):
    class Meta:
        verbose_name = _("secta Event")
        abstract = True
...
class sectaEventIndexPage(sectaWebPage):
    """
    Shows a list of event sub-pages.
    """

    class Meta:
        verbose_name = _("secta Event Index Page")
        abstract = True
...
class sectaEventOccurrence(Orderable, BaseOccurrence):
    class Meta(Orderable.Meta):
        verbose_name = _("secta Event Occurrence")
        abstract = True


class sectaFormMixin(models.Model):
    class Meta:
        abstract = True

    submissions_list_view_class = sectaSubmissionsListView
    ...
    button_style = models.CharField(
        blank=True,
        choices=Secta_settings.Secta_FRONTEND_BTN_STYLE_CHOICES,
        default=Secta_settings.Secta_FRONTEND_BTN_STYLE_DEFAULT,
        max_length=255,
        verbose_name=_("Button style"),
    )
    button_size = models.CharField(
        blank=True,
        choices=Secta_settings.Secta_FRONTEND_BTN_SIZE_CHOICES,
        default=Secta_settings.Secta_FRONTEND_BTN_SIZE_DEFAULT,
        max_length=255,
        ...
        for key, val in form.cleaned_data.items():
            if (
                ...
                processed_data[key] = "{0}{1}".format(
                    Secta_settings.Secta_PROTECTED_MEDIA_URL, path
                )
            else:
    ...
    def get_storage(self):
        return FileSystemStorage(
            location=Secta_settings.Secta_PROTECTED_MEDIA_ROOT,
            base_url=Secta_settings.Secta_PROTECTED_MEDIA_URL,
        )
...
class sectaFormPage(sectaFormMixin, sectaWebPage):
    """
    This is basically a clone of wagtail.contrib.forms.models.AbstractForm
    with changes in functionality and extending sectaWebPage vs wagtailcore.Page.
    """

    class Meta:
        verbose_name = _("secta Form Page")
        abstract = True

    template = "sectacms/pages/form_page.html"
    miniview_template = "sectacms/pages/form_page.mini.html"
    landing_page_template = "sectacms/pages/form_page_landing.html"

    base_form_class = WagtailAdminFormPageForm

    form_builder = sectaFormBuilder

    body_content_panels = (
        sectaWebPage.body_content_panels
        + [
            InlinePanel("form_fields", label="Form fields"),
        ]
        + sectaFormMixin.body_content_panels
        + [
    ...
    settings_panels = (
        sectaPage.settings_panels + sectaFormMixin.settings_panels
    )
...
class sectaSubmissionRevision(SubmissionRevision, models.Model):
    pass


class sectaSessionFormSubmission(SessionFormSubmission):
    ...
    def create_normal_submission(self, delete_self=True):
        ...
        if delete_self:
            sectaSubmissionRevision.objects.filter(
    ...
    def render_link(self, value):
        return "{0}{1}".format(Secta_settings.Secta_PROTECTED_MEDIA_URL, value)
...
def create_submission_changed_revision(sender, **kwargs):
    ...
    sectaSubmissionRevision.create_from_submission(
        submission,
        (
            sectaSubmissionRevision.CREATED
            if created
            else sectaSubmissionRevision.CHANGED
        ),
    )
...
def create_submission_deleted_revision(sender, **kwargs):
    if not issubclass(sender, sectaSessionFormSubmission):
        return
    submission = kwargs["instance"]
    sectaSubmissionRevision.create_from_submission(
...
class sectaSteps(Steps):
    def __init__(self, page, request=None):
        ...
        if has_steps:
            steps = [
                sectaStep(self, i, form_field)
                for i, form_field in enumerate(self.form_fields)
            ]
        else:
            steps = [sectaStep(self, 0, self.form_fields)]
        super(Steps, self).__init__(steps)


class sectaStreamFormMixin(StreamFormMixin):
    class Meta:
        abstract = True

    def get_steps(self, request=None):
        if not hasattr(self, "steps"):
            steps = sectaSteps(self, request=request)
            if request is None:
                return steps
            self.steps = steps
        return self.steps
    ...
    def get_session_submission_class():
        return sectaSessionFormSubmission
...
class sectaStreamFormPage(
    sectaFormMixin, sectaStreamFormMixin, sectaWebPage
):
    class Meta:
        verbose_name = _("secta Advanced Form Page")
        abstract = True
    ...
    def process_form_get(self, form, request):
        return sectaWebPage.serve(self, request)
...
class sectaLocationPage(sectaWebPage):
    """
    Location, suitable for store locations or help centers.
    """

    class Meta:
        verbose_name = _("secta Location")
        abstract = True
    ...
    settings_panels = sectaWebPage.settings_panels + [
        MultiFieldPanel(
            [
    ...
    def render_pin_description(self):
        return render_to_string(
            "sectacms/includes/map_pin_description.html", {"page": self}
        )

    @property
    def render_list_description(self):
        return render_to_string(
            "sectacms/includes/map_list_description.html", {"page": self}
        )
    ...
    def save(self, *args, **kwargs):
        ...
        return super(sectaLocationPage, self).save(*args, **kwargs)
...
class sectaLocationIndexPage(sectaWebPage):
    """
    Shows a map view of the children sectaLocationPage.
    """

    class Meta:
        verbose_name = _("secta Location Index Page")
        abstract = True
