"""
Custom wagtail settings used by Wagtail Secta.
Settings are user-configurable on a per-site basis (multisite).
Global project or developer settings should be defined in sectacms.settings.py .
"""
...
from sectacms.fields import MonospaceField
from sectacms.settings import Secta_settings
from sectacms.models.snippet_models import Navbar, Footer
...
class LayoutSettings(ClusterableModel, BaseSiteSetting):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._meta.get_field("frontend_theme").choices = (
            Secta_settings.Secta_FRONTEND_THEME_CHOICES
        )
        self._meta.get_field("navbar_collapse_mode").choices = (
            Secta_settings.Secta_FRONTEND_NAVBAR_COLLAPSE_MODE_CHOICES
        )
        self._meta.get_field("navbar_color_scheme").choices = (
            Secta_settings.Secta_FRONTEND_NAVBAR_COLOR_SCHEME_CHOICES
        )
        self._meta.get_field("navbar_format").choices = (
            Secta_settings.Secta_FRONTEND_NAVBAR_FORMAT_CHOICES
        )
        # Set default dynamically.
        if not self.id:
            self.frontend_theme = Secta_settings.Secta_FRONTEND_THEME_DEFAULT
            self.navbar_class = Secta_settings.Secta_FRONTEND_NAVBAR_CLASS_DEFAULT
            self.navbar_collapse_mode = (
                Secta_settings.Secta_FRONTEND_NAVBAR_COLLAPSE_MODE_DEFAULT
            )
            self.navbar_color_scheme = (
                Secta_settings.Secta_FRONTEND_NAVBAR_COLOR_SCHEME_DEFAULT
            )
            self.navbar_format = Secta_settings.Secta_FRONTEND_NAVBAR_FORMAT_DEFAULT
    ...
    panels = [
        HelpPanel(
            ...
            content=_(
                ...
                '<a href="https://docs.Secta.dev/wagtail-Secta/how_to/add_tracking_scripts.html" '  # noqa
