...
from sectacms import utils, __version__
from sectacms.blocks import sectaAdvSettings
from sectacms.forms import SearchForm
from sectacms.models.snippet_models import Navbar, Footer
from sectacms.settings import Secta_settings as Secta_settings_obj
from sectacms.settings import get_bootstrap_setting
...
def is_advanced_setting(obj):
    return sectaAdvSettings in (obj.__class__,) + obj.__class__.__bases__
...
def process_form_cell(request, cell):
    if isinstance(cell, str) and cell.startswith(
        Secta_settings_obj.Secta_PROTECTED_MEDIA_URL
    ):
...
@register.filter
def Secta_settings(value):
    return getattr(Secta_settings_obj, value)
