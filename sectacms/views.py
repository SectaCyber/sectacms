...
from sectacms.models import sectaPage, LayoutSettings
from sectacms.importexport import (
    convert_csv_to_json,
    import_pages,
    ImportPagesFromCSVFileForm,
)
from sectacms.settings import Secta_settings
from sectacms.templatetags.sectacms_tags import get_name_of_class
...
def serve_protected_file(request, path):
    ...
    mediapath = os.path.abspath(Secta_settings.Secta_PROTECTED_MEDIA_ROOT)
...
def event_generate_single_ical_for_event(request):
    ...
    # Get the page.
    try:
        event = sectaPage.objects.get(pk=event_pk).specific
    except (sectaPage.DoesNotExist, ValueError):
        raise Http404("Event does not exist")

    # Generate the ical file.
    ical = Calendar()
    ical.add("prodid", "-//Wagtail Secta//")
...
def event_generate_ical_for_calendar(request):
    ...
    # Get the page.
    try:
        page = sectaPage.objects.get(pk=page_id).specific
    except (sectaPage.DoesNotExist, ValueError):
